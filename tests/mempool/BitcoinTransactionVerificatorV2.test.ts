 ...
 *
 * H-04 (reorg follow-up): The original `blockHeight === currentSolutionsHeight`
 *       shortcut would skip the refetch on reorgs that re-emit the same
 *       block height, leaving stale challenges in place. The fix removes the
 *       shortcut so every onBlockChange call triggers a fresh fetch (modulo
 *       in-flight collapse via the targetSolutionsHeight check).
 */
...
describe('BitcoinTransactionVerificatorV2.onBlockChange', () => {
    ...
    describe('basic behavior', () => {
        it('starts with empty initial state', async () => {
            ...
            expect(state.currentSolutionsHeight).toBe(-1n);
        ...
        it('fetches and applies solutions on first onBlockChange', async () => {
            ...
            expect(asPrivate(verificator).currentSolutionsHeight).toBe(100n);
        ...
        it('advances forward through sequential heights', async () => {
            ...
            expect(asPrivate(verificator).currentSolutionsHeight).toBe(102n);
        });
    });
    ...
    describe('reorg correctness', () => {
        it('refetches when called twice with the same height (same-height reorg)', async () => {
            ...
            // state with different challenge solutions. The fix removes the
            // `blockHeight === currentSolutionsHeight` shortcut so every call
        ...
        it('refetches when height regresses (rollback before replay)', async () => {
            ...
            expect(asPrivate(verificator).currentSolutionsHeight).toBe(95n);
    ...
    describe('retry behavior', () => {
        ...
        it('rejects the caller promise after exhausting all retries', async () => {
            ...
            // currentSolutionsHeight must NOT have advanced because the fetch
            // never succeeded.
            expect(asPrivate(verificator).currentSolutionsHeight).toBe(-1n);
        });
        ...
        it('keeps allowedChallenges at the last known-good value after exhaustion', async () => {
            ...
            // currentSolutionsHeight stays at 100 because 101 never succeeded.
            expect(asPrivate(verificator).currentSolutionsHeight).toBe(100n);
        });
    ...
    describe('staleness and abandonment', () => {
        it('abandons a stale fetch when a newer height arrives mid-DB-call', async () => {
            ...
            expect(asPrivate(verificator).currentSolutionsHeight).toBe(101n);
            expect(await asPrivate(verificator).allowedChallenges).toBe(sol101);
        ...
        it('collapses a burst of rapid-fire calls to the latest height', async () => {
            ...
            // Final state must be 104, not 100.
            expect(asPrivate(verificator).currentSolutionsHeight).toBe(104n);
