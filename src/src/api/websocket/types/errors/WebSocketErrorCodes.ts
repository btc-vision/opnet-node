...
/**
 * Check if an error code is a protocol error (should terminate connection)
 */
export function isProtocolError(code: WebSocketErrorCode): boolean {
    return code >= ProtocolError.MALFORMED_MESSAGE && code < AuthError.AUTHENTICATION_REQUIRED;
}

/**
 * Check if an error code is a fatal error (should terminate connection)
 */
export function isFatalError(code: WebSocketErrorCode): boolean {
    // Unknown opcode and malformed messages are fatal
    return (
        code === ProtocolError.UNKNOWN_OPCODE ||
        code === ProtocolError.MALFORMED_MESSAGE ||
        code === ProtocolError.UNSUPPORTED_PROTOCOL_VERSION
    );
}
